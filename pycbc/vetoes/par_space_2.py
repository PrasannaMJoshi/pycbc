...
#----------------------------------------------------------------------------------------------------
# Coordinate transformation function to transform metric coordinates.
def coordinate_transform(coord1, coord2, chirp_m, transform):
    ''' Function for transformation of SG parameter space coordinates. 
    transform = "qf->zy" for (f0, q) ---> (z, y)
                "zy->qf" for (z, y) ---> (f0, q)
    coord1, coord2 = f0, q  if transform == "forward"
                     z, y   if transform == "inverse"
    chirp_m -> Chirp mass of the binary
    '''
    if transform == "qf->zy":
        f0, q = coord1, coord2
        z = (2 * np.pi * f0 * chirp_m)**(-5./3.)
        y = np.log(2 * np.pi * f0 / q)
        return z, y
    elif transform == "zy->qf":
        z, y = coord1, coord2
        f0 = z**(-3./5.) / (2 * np.pi * chirp_m)
        q = z**(-3./5.) / (np.exp(y) * chirp_m)
        return q, f0
    
# Function to sample points in SG parmeter space.
def sample_parameter_space(min_proj, q_range, f0_range, chirp_m):
    ...
    q_range     ->      Bounds on parameter Q
    f0_range    ->      Bounds on parameter f0
    chirp_m     ->      Chirp mass of binary.
    '''

    qmin, qmax = q_range
    f0min, f0max = f0_range
    # Transform the bounds on the parameter space to z-y coordinate system.
    z1, y1 = coordinate_transform(f0min, qmin, chirp_m, "qf->zy")
    z2, y2 = coordinate_transform(f0min, qmax, chirp_m, "qf->zy")
    z3, y3 = coordinate_transform(f0max, qmin, chirp_m, "qf->zy")
...
##---------------------------------------------------------------------------------------------------
##---------------------------------------------------------------------------------------------------
# To be deleted before submitting to MR.
##---------------------------------------------------------------------------------------------------
##---------------------------------------------------------------------------------------------------
# Old wrapper function for sampling points in parameter space.
...
# Old coordinate transformation functions.
...
# Function that calcultes min_proj based in mass of trigger.
